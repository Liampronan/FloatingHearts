...
public class HeartView: UIView {
    ...
    private func performSlightRotationAnimation(direction: RotationDirection) {
        ...
        HeartView.animate(withDuration: Durations.Full) {
...
//        let heartImage = UIImage(systemName: Self.sysImageName)
//        Self.sysImageName = (Self.sysImageName == "heart.fill" ? "music.note" : "heart.fill")
...
//        heartImage?.draw(in: rect, blendMode: .normal, alpha: 1.0)
        // Fetch the system image
           if let heartImageSystem = UIImage(systemName: Self.sysImageName) {
               // Create a new image by tinting the original image
               let heartImage = heartImageSystem.withTintColor(theme.fillColor, renderingMode: .alwaysOriginal)
               heartImage.draw(in: rect, blendMode: .normal, alpha: 1.0)
               Self.sysImageName = (Self.sysImageName == "heart.fill" ? "music.note" : "heart.fill")
           }
#else
